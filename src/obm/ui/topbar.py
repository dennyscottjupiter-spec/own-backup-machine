...
class TopBar(ctk.CTkFrame):
    def __init__(
        ...
        ctk.CTkButton(self, text="Rescan", command=on_scan, fg_color=theme.ACCENT).pack(
            side="right", padx=16, pady=10
        )
        if on_settings is not None:
            ctk.CTkButton(self, text="Settings", command=on_settings, fg_color=theme.PANEL_BG).pack(
                side="right", padx=4, pady=10
            )
        if on_history is not None:
            ctk.CTkButton(self, text="History", command=on_history, fg_color=theme.PANEL_BG).pack(
                side="right", padx=4, pady=10
            )
